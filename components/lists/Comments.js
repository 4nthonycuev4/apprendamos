...
export default function CommentList({ contentRef, viewer }) {
    ...
    useEffect(() => {
        const getComments = async () => {
            const comments = await fetch(
                `/api/${contentRef.collection}/${contentRef.id}/comments`
            )
                .then((res) => res.json())
    ...
    const getNextComments = async () => {
        const nextComments = await fetch(
            `/api/${contentRef.collection}/${contentRef.id}/comments?afterId=${afterRef.id}`
        )
            .then((res) => res.json())
            .catch((err) => console.log(err));
    ...
    return (
        <div className="space-y-2">
            ...
            {afterRef && afterRef.id && (
                <button type="button" onClick={getNextComments}>
                    más comentario
                </button>
            )}
            {commentSelected && (
