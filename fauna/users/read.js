...
export const GetFollowingStatus = (username) => Let(
  {
    viewerRef: GetViewerRef(),
    user: GetUserByUsername(username),
    authorUserRelMatch: Match(Index("author_user_rel"), [Select(["ref"], Var("user")), Var("viewerRef")]),
    following: If(Exists(Var("authorUserRelMatch")), Select(["data", "following"], Get(Var("authorUserRelMatch")), false), false),
  },
  {
...
export const StalkUser = (username) => Let(
  {
...
export const GetUserByUsername = (username) => Get(Match(Index("user_by_username"), [username]));

export const GetUserRefByUsername = (username) => Select(["ref"], GetUserByUsername(username));

export const GetSingleUser = (username) => Let(
  {
    user: GetUserByUsername(username),
  },
  {
...
export function GetViewerRef() {
  return If(
    HasCurrentIdentity(),
    Select(["data", 0], Paginate(
      Match(
        Index("user_by_connection"),
        CurrentIdentity()
      )
    )),
    null
...
export const GetPartialUser = (userRef) => Let(
  {
    user: Get(userRef),
  },
  {
...
export const GetTrendingUsers = (afterRef) => Map(
  Paginate(
    Join(
