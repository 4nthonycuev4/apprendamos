    ...
    return Let(
        {
            ...
            publicationStats: Get(Match(
                Index("publicationstats_by_user"),
                [publicationRef, Var("viewerRef")]
            )),
            ...
            publicationStatsUpdated: Update(Select(["ref"], Var("publicationStats")), {
                data: {
                    like: Not(Var("like")),
                    dislike: false,
                }
            ...
            publicationUpdated: Update(publicationRef, {
                data: {
                    stats: {
                        likeCount: Add(Select(["data", "stats", "likeCount"], Var("publication")), Var("likeGain")),
                        dislikeCount: Add(Select(["data", "stats", "dislikeCount"], Var("publication")), Var("dislikeGain")),
                    }
                }
            }),
        ...
        {
            viewerStats: {
                like: Select(["data", "like"], Var("publicationStatsUpdated"), false),
                dislike: Select(["data", "dislike"], Var("publicationStatsUpdated"), false),
                save: Select(["data", "save"], Var("publicationStatsUpdated"), false),
                comment: If(Select(["data", "commentCount"], Var("publicationStatsUpdated"), 0) > 0, true, false),
                cheer: If(Select(["data", "cheerCount"], Var("publicationStatsUpdated"), 0) > 0, true, false),
            },
            stats: {
                likeCount: Select(["data", "stats", "likeCount"], Var("publicationUpdated"), 0),
                commentCount: Select(["data", "stats", "commentCount"], Var("publicationUpdated"), 0),
                cheerCount: Select(["data", "stats", "cheerCount"], Var("publicationUpdated"), 0),
            }
        }
    )
}
    ...
    return Let(
        {
            ...
            publicationUpdated: Update(publicationRef, {
                data: {
                    stats: {
                        saveCount: Add(Select(["data", "stats", "saveCount"], Var("publication")), Var("saveGain")),
                    }
                }
            }),
            authorStatsUpdated: Update(Select(["ref"], Var("authorStats")), {
    ...
    Let(
        {
            ...
            publicationUpdated: Update(publicationRef, {
                data: {
                    stats: {
                        viewCount: Add(Select(["data", "stats", "viewCount"], Var("publication"), 0), 1),
                    }
                }
            }),

            publicationStatsMatch: Match(
                Index("publicationstats_by_user"),
                [publicationRef, Var("viewerRef")]
            ),


            publicationStats: Var("viewerRef") && If(
                Exists(Var("publicationStatsMatch")),
                Let(
                    {
                        originalPublicationStats: Get(Var("publicationStatsMatch")),
                    },
                    Update(Select(["ref"], Var("publicationStats")), {
                        data: {
                            viewCount: Add(Select(["data", "viewCount"], Var("originalPublicationStats"), 0), 1),
                        }
                    })
                ),
